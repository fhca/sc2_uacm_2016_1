...
to setup
  ca  ; borra el mundo
  set entrada (patch-set patch -14 14 patch -13 14 patch -12 14)
  set salida (patch-set patch 14 14 patch 13 14 patch 12 14)
  set cajas patch-set patches with [pycor = 12 and pxcor > -5 and pxcor < 5 ]
  set bases (patch-set patch -7 -2 patch 0 -2 patch 7 -2)
  set productos (list 30 90 110 130 20 120    70 80 0) ; los últimos no los tendrá la tienda
  ask entrada [
    set pcolor green
  ]
  ask salida [
...
to anaquel [base] ; base = parche central
  ask patches with [
    ...
    pycor > [pycor - 10] of base and pycor < [pycor + 10] of base]
  [
    set pcolor gray
  ]
...
to poner-productos [base]
  ...
  [
    let x [pxcor] of base
    let derecha (pxcor - x + 1) / 2
    set x (x / 7 + 1) * 2 + derecha
    let y pycor - ([pycor] of base) + 10
    set pcolor item x productos + (0.5 * y)
    set cant-producto 50
  ]
end
...
to mueve-tortugas
  ask turtles [
    fd 1
    let tolerancia (random 2) + 1
    if distance item bandera-siguiente banderas <= tolerancia [
      if bandera-siguiente < length banderas - 1 [
        set bandera-siguiente bandera-siguiente + 1
